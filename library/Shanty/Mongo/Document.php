...
use Shanty\Mongo as Mongo,
  Shanty\Mongo\Exception as ShantyException,
  Shanty\Mongo\Collection,
  Shanty\Mongo\Iterator\DefaultIterator;
...
class Document extends Collection implements \ArrayAccess, \Countable, \IteratorAggregate
{
	...
	public function _getMongoDb($writable = true)
	{
		if (is_null($this->getConfigAttribute('db'))) {
			throw new ShantyException('Can not fetch instance of MongoDb. Document is not connected to a db.');
		}

		if ($writable) $connection = Mongo::getWriteConnection($this->getConfigAttribute('connectionGroup'));
		else $connection = Mongo::getReadConnection($this->getConfigAttribute('connectionGroup'));
	...
	public function _getMongoCollection($writable = true)
	{
		if (is_null($this->getConfigAttribute('collection'))) {
			throw new ShantyException('Can not fetch instance of MongoCollection. Document is not connected to a collection.');
		}
		...
		switch($requirement) {
			...
			case 'DocumentSet':
				foreach ($this->_docRequirements[$property] as $requirementSearch => $params) {
					$standardClass = '\Shanty\Mongo\\'.$requirement;
					...
					if (!empty($matches)) {
						if (!class_exists($matches[1])) {
							throw new ShantyException("$requirement class of '{$matches[1]}' does not exist");
						}

						if (!is_subclass_of($matches[1], $standardClass)) {
							throw new ShantyException("$requirement of '{$matches[1]}' sub is not a class of $standardClass does not exist");
						}
	...
	public function getValidators($property)
	{
		...
		foreach ($this->_docRequirements[$property] as $requirement => $options) {
			...
			$validator = Mongo::retrieveRequirement($requirement, $options);
			if (!$validator || !($validator instanceof \Zend\Validator\AbstractValidator)) continue;
	...
	public function getFilters($property)
	{
		...
		foreach ($this->_docRequirements[$property] as $requirement => $options) {
			...
			$filter = Mongo::retrieveRequirement($requirement, $options);
			if (!$filter || !($filter instanceof \Zend\Filter\AbstractFilter)) continue;
	...
	public function setProperty($property, $value)
	{
		if (substr($property, 0, 1) == '_') {
			throw new ShantyException("Can not set private property '$property'");
		}
		...
		if (!is_null($value) && !$validators->isValid($value)) {
			throw new ShantyException(implode($validators->getMessages(), "\n"));
		}
		...
		if ($value instanceof Shanty\Mongo\Document && !$this->hasRequirement($property, 'AsReference')) {
	...
	public function getPropertyKeys()
	{
		...
		foreach ($this->_data as $property => $value) {
			if (($value instanceof Document && !$value->isEmpty()) ||
				(!($value instanceof Document) && !is_null($value))) {
				$keyList[] = $property;
			}
			else {
				$doNoCount[] = $property;
	...
	public function createReference()
	{
		if (!$this->isRootDocument()) {
			throw new ShantyException('Can not create reference. Document is not a root document');
		}

		if (!$this->isConnected()) {
			throw new ShantyException('Can not create reference. Document does not connected to a db and collection');
		}
	 ...
	 *
	 * @param Shanty\Mongo\Document $document
	 * @return boolean
	 */
	public function isReference(Document $document)
	{
	...
	public function export()
	{
		...
		foreach ($this->_data as $property => $value) {
			...
			if ($value instanceof Document) {
				...
				if ($this instanceof DocumentSet) {
					$requirementIndex = DocumentSet::DYNAMIC_INDEX;
				}
				else {
		...
		foreach ($requiredProperties as $property) {
			if (!isset($exportData[$property]) || (is_array($exportData[$property]) && empty($exportData[$property]))) {
				throw new ShantyException("Property '{$property}' must not be null.");
			}
		}
	...
	{
		if (!$this->isConnected()) {
			throw new ShantyException('Can not save documet. Document is not connected to a db and collection');
		}

		if ($this->isLocked()) {
			throw new ShantyException('Can not save documet. Document is locked.');
		}
	...
	{
		if (!$this->isConnected()) {
			throw new ShantyException('Can not delete document. Document is not connected to a db and collection');
		}
	 ...
	 *
	 * @return Shanty\Mongo\DefaultIterator
	 */
	...
	public function addOperation($operation, $property = null, $value = null)
	{
		...
		if (!Mongo::isValidOperation($operation)) {
			throw new ShantyException("'{$operation}' is not valid operation");
		}
