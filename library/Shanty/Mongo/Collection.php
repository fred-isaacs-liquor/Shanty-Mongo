...
use Shanty\Mongo\Document,
  ...
  Shanty\Mongo\Iterator\Cursor,
  Shanty\Mongo\DocumentSet;
...
abstract class Collection
{
	...
	protected static $_documentSetClass = 'DocumentSet';
	...
	{
		return is_subclass_of(get_called_class(), 'Document');
	}
	...
	public static function getDocumentClass()
	{
		if (!static::isDocumentClass()) {
			throw new Shanty\Mongo\Exception(get_called_class().' is not a document. Please extend Shanty\Mongo\Document');
		}
	...
	{
		if ($writable) $connection = Mongo::getWriteConnection(static::getConnectionGroupName());
		else $connection = Mongo::getReadConnection(static::getConnectionGroupName());
	...
	{
		if (!static::hasDbName()) {
			throw new ShantyException(get_called_class().'::$_db is null');
		}
	...
	public static function getMongoCollection($writable = true)
	{
		if (!static::hasCollectionName()) {
			throw new ShantyException(get_called_class().'::$_collection is null');
		}
	...
	{
		if (isset($data['_type']) && is_array($data['_type']) && class_exists($data['_type'][0]) && is_subclass_of($data['_type'][0], 'Document')) {
	...
	public static function all(array $query = array(), array $fields = array())
	{
		...
		return new Cursor($cursor, $config);
	}
